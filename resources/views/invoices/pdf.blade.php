...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .invoice-title { font-size: 24px; font-weight: bold; color: {{ $primaryColor }}; margin-bottom: 5px; }
        .invoice-number { font-size: 14px; color: #666; }
        .parties { display: flex; gap: 16px; margin-bottom: 20px; }
        .party { flex: 1; padding: 12px; border: 1px solid #e5e7eb; border-radius: 4px; }
        .party-label { font-size: 8px; text-transform: uppercase; color: #9ca3af; letter-spacing: 1px; margin-bottom: 6px; }
        .party-name { font-size: 12px; font-weight: bold; margin-bottom: 4px; }
        .dates-bank-row { display: flex; gap: 16px; margin-bottom: 20px; }
        .dates { width: 32%; padding: 12px; background: #fff; border: 1px solid #e5e7eb; border-radius: 4px; }
        .date-item { margin-bottom: 8px; }
        .date-item:last-child { margin-bottom: 0; }
        .date-label { font-size: 8px; text-transform: uppercase; color: #9ca3af; }
        .date-value { font-size: 11px; font-weight: bold; }
        .bank-info { flex: 1; padding: 12px; background: {{ $lightBg }}; border: 1px solid {{ $lightBorder }}; border-radius: 4px; display: flex; justify-content: space-between; align-items: flex-start; }
        .bank-info-details { flex: 1; }
        .bank-info-title { font-size: 10px; font-weight: bold; margin-bottom: 4px; }
        .bank-info-qr { margin-left: 16px; text-align: center; }
        .bank-info-qr img { width: 120px; height: 120px; }
        .bank-info-qr .qr-label { font-size: 7px; color: #6b7280; margin-top: 2px; }
        table.items { width: 100%; border-collapse: collapse; margin-bottom: 20px; }
        table.items th { background: {{ $primaryColor }}; color: white; padding: 8px 6px; font-size: 9px; text-transform: uppercase; text-align: left; }
        table.items td { padding: 7px 6px; border-bottom: 1px solid #e5e7eb; font-size: 10px; }
    ...
    <div class="container">
        ...
        <div class="parties">
            <div class="party">
                ...
                @if(!empty($seller['street']))<div>{{ $seller['street'] }}</div>@endif
                @if(!empty($seller['zip']) || !empty($seller['city']))
                    <div>{{ $seller['zip'] ?? '' }} {{ $seller['city'] ?? '' }}</div>
                @endif
                @if(!empty($seller['business_number']))<div style="margin-top:4px">{{ __('invoice.ico') }}: {{ $seller['business_number'] }}</div>@endif
                @if(!empty($seller['tax_number']))<div>{{ __('invoice.dic') }}: {{ $seller['tax_number'] }}</div>@endif
                @if(!empty($seller['vat_number']))<div>{{ __('invoice.ic_dph') }}: {{ $seller['vat_number'] }}</div>@endif
                @if(!empty($seller['email']))<div style="margin-top:4px">{{ $seller['email'] }}</div>@endif
            ...
            <div class="party">
                ...
                @if(!empty($buyer['company_name']))<div>{{ $buyer['company_name'] }}</div>@endif
                @if(!empty($buyer['street']))<div>{{ $buyer['street'] }}</div>@endif
                @if(!empty($buyer['zip']) || !empty($buyer['city']))
                    <div>{{ $buyer['zip'] ?? '' }} {{ $buyer['city'] ?? '' }}</div>
                @endif
                @if(!empty($buyer['business_number']))<div style="margin-top:4px">{{ __('invoice.ico') }}: {{ $buyer['business_number'] }}</div>@endif
                @if(!empty($buyer['tax_number']))<div>{{ __('invoice.dic') }}: {{ $buyer['tax_number'] }}</div>@endif
                @if(!empty($buyer['vat_number']))<div>{{ __('invoice.ic_dph') }}: {{ $buyer['vat_number'] }}</div>@endif
                @if(!empty($buyer['email']))<div style="margin-top:4px">{{ $buyer['email'] }}</div>@endif
                @if(!empty($buyer['phone']))<div>{{ $buyer['phone'] }}</div>@endif
                @if(!empty($buyer['web']))<div>{{ $buyer['web'] }}</div>@endif
            </div>
        </div>
        ...
        <div class="dates-bank-row">
            ...
            <div class="bank-info">
                <div class="bank-info-details">
                    ...
                    @if(!empty($seller['bank_name']))<div>{{ __('invoice.bank_name') }}: {{ $seller['bank_name'] }}</div>@endif
                    @if(!empty($seller['bank_account_number']))<div>{{ __('invoice.account_number') }}: {{ $seller['bank_account_number'] }}</div>@endif
                    @if(!empty($seller['bank_iban']))<div>{{ __('invoice.iban') }}: {{ $seller['bank_iban'] }}</div>@endif
                    @if(!empty($seller['bank_swift']))<div>{{ __('invoice.swift') }}: {{ $seller['bank_swift'] }}</div>@endif
                    <div>{{ __('invoice.variable_symbol') }}: {{ preg_replace('/\D/', '', $invoice->invoice_number) }}</div>
                </div>
                @if(!empty($qrBase64))
